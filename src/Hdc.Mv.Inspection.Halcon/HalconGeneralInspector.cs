...
namespace Hdc.Mv.Inspection
{
    public class HalconGeneralInspector : IGeneralInspector
    {
        ...
        public InspectionResult Inspect(ImageInfo imageInfo, InspectionSchema inspectionSchema)
        {
            ...
            var edgeSearchingDefinitions = inspectionSchema.EdgeSearchingDefinitions;

            //            {
            //                GetValue(edgeSearchingDefinitions, inspectionResult);
            //            }

            {
                var edges = SearchEdges(inspectionSchema.EdgeSearchingDefinitions);

                inspectionResult.Edges = edges;

                //                var results = GetDistanceBetweenPointsResults(edges);
                var results = GetDistanceBetweenPointsResults2(edges);
